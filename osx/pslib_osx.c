...
static CpuTimes* per_cpu_times() {
    ...
    kerror = host_processor_info(host_port,
                                 PROCESSOR_CPU_LOAD_INFO,
                                 &cpu_count,
                                 &info_array,
                                 &info_count);
    check(kerror == KERN_SUCCESS,
    ...
    sysret = vm_deallocate(
        mach_task_self(), (vm_address_t)info_array, info_count * pagesize);
...
CpuTimes* cpu_times(bool percpu) {
    ...
    if (!percpu) {
        ...
        mach_port_t host_port = mach_host_self();
        kerror = host_statistics(
            host_port, HOST_CPU_LOAD_INFO, (host_info_t)&r_load, &count);
        check(kerror == KERN_SUCCESS,
