...
static struct dict_t* parse_to_dict(char* buffer,
                                    const char** reserved,
                                    size_t token_count,
                                    const char* delim) {
    char* token = NULL;
...
static char* procfs_read(const char* path) {
    char* content = NULL;
    char buffer[256];
...
static double sum_cpu_time(CpuTime* t) {
    double ret = 0.0;
    ret += t->user;
    ret += t->system;
    ret += t->idle;
...
static CpuTime* calculate_cpu_times_percentage(CpuTime* t1, CpuTime* t2) {
    CpuTime* ret;
    double all_delta = sum_cpu_time(t2) - sum_cpu_time(t1);
    ret = (CpuTime*)calloc(1, sizeof(CpuTime));
    check_mem(ret);
...
static double calculate_cpu_util_percentage(CpuTime* t1, CpuTime* t2) {
...
int parse_cpu_times(char* line, CpuTime* ret) {
...
DiskPartitionInfo* disk_partitions(bool physical) {
    ...
    while ((entry = getmntent(file))) {
        if (physical && !lfind(&entry->mnt_type,
                               phys_devices,
                               &nphys_devices,
                               sizeof(char*),
                               str_comp)) {
        ...
        d->fs_type = strdup(entry->mnt_type);
        ...
        if (ret->nitems == nparts) {
            nparts *= 2;
            partitions = (DiskPartition*)realloc(
                partitions, sizeof(DiskPartition) * nparts);
            check_mem(partitions);
            ret->partitions = partitions;
            d = ret->partitions + ret->nitems; /* Move the cursor to the correct
                                                  value in case the realloc
                                                  moved the memory */
        }
    }
    endmntent(file);
...
DiskIOCounterInfo* disk_io_counters() {
    ...
    while (fgets(line, 40, fp) != NULL) {
        ...
        tmp = strtok(NULL, " \n"); /* Number of blocks */
        tmp = strtok(NULL, " \n"); /* Name */
        if (isdigit(tmp[strlen(tmp) - 1])) { /* Only partitions (i.e. skip
                                                things like 'sda')*/
...
error:
    ...
    if (ret)
        free_disk_io_counter_info(ret);

    return NULL;
}

void free_disk_io_counter_info(DiskIOCounterInfo* di) {
...
void free_net_io_counter_info(NetIOCounterInfo* d) {
    uint32_t i = 0;
    for (; i < d->nitems; i++) {
...
UsersInfo* get_users() {
    ...
    while (NULL != (ut = getutent())) {
        ...
        if (ret->n_items ==
            ...
            check_mem(users);
            ret->users = users;
            u = ret->users + ret->n_items; /* Move the cursor to the correct
...
#define ULLONG_MAX (~0ULL)
    ...
    if (cached == ULLONG_MAX || active == ULLONG_MAX ||
        inactive == ULLONG_MAX) {
        log_warn(
...
bool swap_memory(SwapMemInfo* ret) {
    ...
    if (sin == ULONG_MAX || sout == ULONG_MAX) {
        log_warn(
            "Couldn't determine 'sin' and 'sout' swap stats. Setting them to "
            "0");
        sout = sin = 0;
    }
    fclose(fp);
...
CpuTime* cpu_times(bool per_cpu) {
    ...
    if (!per_cpu) {
        ...
        ret = (CpuTime*)calloc(20, sizeof(CpuTime));
        check_mem(ret);
...
CpuTimeList* cpu_times_s(bool per_cpu) {
    ...
    ctl = calloc(1, sizeof(CpuTimeList));
    check_mem(ctl);
...
CpuTime* cpu_times_percent(bool per_cpu, CpuTime* prev_times) {
    CpuTime* current = NULL;
    CpuTime* t;
    int i, ncpus = per_cpu ? cpu_count(1) : 1;
    CpuTime* ret;
    check(prev_times, "Need a reference point. prev_times can't be NULL");
    current = cpu_times(per_cpu);
    check(current, "Couldn't obtain CPU times");
    ret = (CpuTime*)calloc(ncpus, sizeof(CpuTime));
    check_mem(ret);
    for (i = 0; i < ncpus; i++) {
...
double* cpu_util_percent(bool per_cpu, CpuTime* prev_times) {
    ...
    for (i = 0; i < ncpus; i++) {
        percentage[i] =
            calculate_cpu_util_percentage(prev_times + i, current + i);
    }
    free(current);
    return percentage;
error:
...
Process* get_process(pid_t pid) {
    /* TODO: Add test for invalid pid. Right now, we get a lot of errors and
     * some structure.*/
    ...
    p->pid = pid;
    p->ppid = get_ppid(pid);
    p->name = get_process_name(pid);
    p->exe = get_exe(pid);
    p->cmdline = get_cmdline(pid);
    p->create_time = get_create_time(pid);
    uids = get_ids(pid, "Uid:");
    if (uids) {
        p->uid = uids[0];
        p->euid = uids[1];
        p->suid = uids[2];
        p->username = get_username(p->uid); /* Uses real uid and not euid */
    } else {
...
double process_cpu_util_percent(pid_t pid) {
    ...
    if (now <= lastCPU || time_sample.tms_stime < lastSysCPU ||
        time_sample.tms_utime < lastUserCPU) {
        percent = -1.0;
    } else {
        percent = (time_sample.tms_stime - lastSysCPU) +
                  (time_sample.tms_utime - lastUserCPU);
        percent /= (now - lastCPU);
        percent /= cpu_count(false);
        percent *= 100;
    }
    return percent;
}
...
bool process_memory(pid_t pid, ProcessMemInfo* mem) {
    check(mem, "Need a reference point. mem can't be NULL")
        const char* process_mem_stat_format = "/proc/%d/statm";
    char process_men_stat[NAME_MAX] = {0};
    sprintf(process_men_stat, process_mem_stat_format, pid);

    FILE* fp = fopen(process_men_stat, "r");
    check(fp, "Couldn't open /proc/%d/statm", pid);

    char line[128];
    check(fgets(line, 140, fp), "read line error!!!");
    fclose(fp);
    fp = NULL;

    char* tmp;
    tmp = strtok(line, " ");
    mem->virtual_size = atoll(tmp);
    tmp = strtok(NULL, " ");
    mem->resident_size = atoll(tmp);
    tmp = strtok(NULL, " ");
    mem->shared_size = atoll(tmp);
    tmp = strtok(NULL, " ");
    mem->trs = atoll(tmp);
    tmp = strtok(NULL, " ");
    mem->lrs = atoll(tmp);
    ...
    mem->drs = atoll(tmp);
    tmp = strtok(NULL, " ");
    mem->dirty_size = atoll(tmp);

    struct sysinfo info;
    sysinfo(&info);

    mem->virtual_size = (mem->virtual_size << 2);
    mem->resident_size = (mem->resident_size << 2);
    mem->shared_size = (mem->shared_size << 2);
    mem->dirty_size = (mem->dirty_size << 2);

    mem->mem_rate = (mem->resident_size * 1.0) / (info.totalram >> 10);
...
ARRAY_ProcessPtr* get_processes() {
    DIR* dir_handle = NULL;
    ARRAY_ProcessPtr* pl = NULL;
    pl = ProcessPtr_init(64);
    if (NULL == pl) {
        goto error;
    }
    ...
    struct dirent* some = NULL;
    while (NULL != (some = readdir(dir_handle))) {
        if (0 == strcmp(some->d_name, "..") || 0 == strcmp(some->d_name, ".") ||
            DT_DIR != some->d_type) {
            continue;
        }

        pid_t pid = atoi(some->d_name);
        if (pid <= 0) {
            continue;
        }

        // printf("-- pid: %s\n", some->d_name);
        Process* process = get_process(atoi(some->d_name));
        ProcessPtr_append(pl, process);
    }
...
ARRAY_Thread* get_threads(pid_t pid) {
    ...
    char tasks[64];
    sprintf(tasks, "/proc/%d/task", pid);
    dir_handle = opendir(tasks);
    if (NULL == dir_handle) {
        goto error;
    }
    ...
    while (NULL != (some = readdir(dir_handle))) {
        ...
        Thread t_info;
        t_info.tid = tid;
        t_info.name = get_process_name(tid);
        Thread_append(tl, t_info);
    }
